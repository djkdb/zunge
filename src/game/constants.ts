...
/*
 * 서버 상한은 딱 잘리는 벽이 아니라 완만한 언덕이다.
 *
 * 벽으로 두면 상한에 닿는 순간 유입 관련 모든 것이 조용히 0 이 된다.
 * 인터넷 업그레이드도, "사용자 +30%" 이벤트도, 출시 보상도 아무 일이 없다.
 * 화면에는 보상이 떴는데 숫자가 그대로인 건 게임이 거짓말을 하는 것과 같다.
 *
 * 그래서 상한을 넘어서도 사용자가 늘 수 있게 하되, 넘어선 만큼 유입이
 * 선형으로 줄어 상한의 USER_OVERLOAD_MAX 배에서 멈춘다.
 * 서버는 여전히 중요하다 — 과부하 구간에 빨리 들어갈수록 유입이 말라붙는다.
 */
export const USER_OVERLOAD_MAX = 2;
