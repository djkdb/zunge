    ...
    <Modal open onClose={() => actions.closeDaily()}>
      <div className="card relative overflow-hidden p-5 text-center">
        ...
        <div className="relative">
          ...
          <h3 className="mt-1 flex items-center justify-center gap-2 text-lg font-black"><Icon name="gift" size={19} className="text-gold" />출석 보상</h3>
          ...
          <div className="my-4 flex justify-center gap-1.5">
            {Array.from({ length: DAILY_MAX_STREAK }).map((_, i) => {
              ...
              return (
                ...
                >
                  <Icon name={done ? 'check' : 'gift'} size={15} strokeWidth={done ? 2.6 : 1.7} />
                  <span>{day}일</span>
                </div>
              );
            })}
          ...
          {available ? (
            ...
            <>
              ...
              <Button block variant="neutral" size="lg" className="mt-4" onClick={() => actions.closeDaily()}>닫기</Button>
            </>
          )}
        </div>
      </div>
