...
게임을 열고 **설정 → 캐릭터 이미지** 에서 8열 x 4행(32포즈) 시트를
끌어다 놓거나 붙여넣기(Ctrl/⌘+V)하면 끝난다.

브라우저가 그 자리에서 체크무늬 배경을 지우고, 번호 라벨을 잘라내고,
32개 포즈로 나눠 IndexedDB 에 저장한다. 새로고침해도 유지된다.
칸마다 캐릭터가 정확히 1명인지 검사해서 결과를 보여준다.

이 방식은 그 브라우저에만 저장된다. 배포본에 포함하려면 방법 2를 쓴다.
...
두 방법 모두 없으면 폴백 픽셀 스프라이트로 자동 전환되므로 게임은 정상 동작한다.
